...
The package includes a comprehensive ERT test suite in =kelly-test.el= covering the core calculation, expectation, wager sizing, odds conversions, dispatch logic, and integration scenarios.  A =Makefile= provides convenient targets for running the suite.

** Running tests locally
:PROPERTIES:
:CUSTOM_ID: h:running-tests-locally
:END:
...
A GitHub Actions workflow (=.github/workflows/test.yml=) runs byte-compilation and the test suite on every push and pull request against the =main= branch.  The workflow tests against Emacs 27.1, 29.4, and 30.1.
